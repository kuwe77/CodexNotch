...
public final class TelemetryCoordinator: ObservableObject {
    ...
    private func handleCodexSseEvent(attributes: [String: TelemetryAttributeValue], endTime: Date, source: TelemetrySource) {
        ...
        let hasActiveCodexWork = isAgentActive
            || activeCodexResponseStart != nil
            || !activeCodexToolCalls.isEmpty
            || !pendingToolDecisions.isEmpty
        let isTerminalEvent = normalizedKind.contains(".done")
            || normalizedKind.contains("_done")
            || normalizedKind.contains(".completed")
        let shouldTreatAsActivity = !isTerminalEvent || hasActiveCodexWork

        guard shouldTreatAsActivity else {
            return
        }
