...
struct CodexNotchContentView: View {
    ...
    private let completionNotificationFreshness: TimeInterval = 10.0
    private let expandedHeaderHeight: CGFloat = 42
    ...
    private var headerHeight: CGFloat {
        isExpanded ? expandedHeaderHeight : notchVM.closedNotchSize.height
    }
    ...
    private var hasJSONLBackedActivity: Bool {
    ...
    var body: some View {
        VStack(spacing: 0) {
            notchBody
                ...
                .padding([.horizontal, .bottom], isExpanded ? 8 : 0)
                ...
                .onChange(of: codexManager.lastTaskCompletion?.id) { _, _ in
                    handleCodexTaskCompletion()
                }
    ...
    private var notchBody: some View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if notchVM.notchState == .open {
                expandedContent
                    .frame(height: max(0, notchVM.notchSize.height - currentHeaderHeight))
    ...
    private var closedHeader: some View {
        ...
        let codexRateLimits = codexManager.currentRateLimits
        ...
        let showCodexUsage = settings.showCodexUsageInNotch
            && settings.enableCodexJSONL
            && codexRateLimits.hasData
            && (activeSource == .codex || !isClaudeActive)
        ...
        // Calculate compact wing widths.
        let toolNameWidth: CGFloat = currentToolName.map { CGFloat(min($0.count, 12)) * 6 } ?? 0
        let thinkingWidth: CGFloat = 58
        let leftWingWidth: CGFloat = showLeftWing
            ? (hasCurrentTool ? 38 + toolNameWidth : (isThinking ? 38 + thinkingWidth : (hasSessions ? 52 : 48)))
            : 0

        let baseRightWingWidth: CGFloat = (hasCurrentTool || isThinking) ? 88
            : (hasPermissionNeeded ? 78 : 0)
        let rightWingWidth: CGFloat = max(baseRightWingWidth, showCodexUsage ? 88 : 0)
        ...
        if !hasAnyContent {
            ...
            HStack(spacing: 0) {
                ...
                if rightWingWidth > 0 {
                    Group {
                        if showCodexUsage {
                            CodexUsageCompactView(rateLimits: codexRateLimits)
                        } else if let codexTool = currentCodexTool, isCodexActive || codexActiveTool != nil {
                            // Codex tool info
                            HStack(spacing: 4) {
            ...
            if !shouldShowMemeVideo {
                ...
                if settings.showTodoList && !claudeCodeManager.state.todos.isEmpty {
                    NotchSection(title: "Current Tasks") {
                        TodoListView(todos: claudeCodeManager.state.todos, maxItems: 2)
                    }
                }
                ...
                if settings.toolDisplayMode == "singular" {
                    ...
                    if isCodexActive && !codexTools.isEmpty {
                        NotchSection(title: "Recent Tools (Codex)") {
                            CodexToolListView(tools: codexTools, maxItems: 2)
                        }
                    } else if !claudeTools.isEmpty {
                        NotchSection(title: "Recent Tools") {
                            ClaudeToolListView(tools: claudeTools, maxItems: 2)
                        }
                    } else {
                        NotchSection(title: "Recent Tools") {
                ...
                if settings.showCodexUsageInNotch && codexRateLimits.hasData {
                    CodexUsageInlineView(rateLimits: codexRateLimits)
                }
            }
    ...
    private enum CodexUsageStyle {
        static func color(for window: CodexRateLimitWindow) -> Color {
            switch window.clampedUsedPercent {
            case 85...:
        ...
        static func resetSuffix(for window: CodexRateLimitWindow) -> String {
            guard let reset = window.resetCountdownText else { return "" }
    ...
    private struct CodexUsageCompactView: View {
        ...
        var body: some View {
            HStack(spacing: 5) {
                if let primary = rateLimits.primary {
                    usageText(label: "5h", window: primary)
                }

                if let secondary = rateLimits.secondary {
                    usageText(label: "1w", window: secondary)
                }
            }
            .padding(.horizontal, 6)
            .padding(.vertical, 2)
        ...
        private func usageText(label: String, window: CodexRateLimitWindow) -> some View {
            HStack(spacing: 2) {
                Text(label)
                    .font(.system(size: 9, weight: .semibold, design: .monospaced))
                    .foregroundColor(.white.opacity(0.55))

                Text(window.formattedRemainingPercent)
                    .font(.system(size: 9, weight: .medium, design: .monospaced))
                    .foregroundColor(CodexUsageStyle.color(for: window))
            }
        }

        private var helpText: String {
            [
                rateLimits.primary.map { "5h: \($0.formattedRemainingPercent) remaining\(CodexUsageStyle.resetSuffix(for: $0))" },
                rateLimits.secondary.map { "1w: \($0.formattedRemainingPercent) remaining\(CodexUsageStyle.resetSuffix(for: $0))" }
            ]
            .compactMap { $0 }
            .joined(separator: "\n")
        }
    ...
    private struct CodexUsageInlineView: View {
        let rateLimits: CodexRateLimits

        var body: some View {
            HStack(spacing: 8) {
                if let primary = rateLimits.primary {
                    usagePill(label: "5h", window: primary)
                }

                if let secondary = rateLimits.secondary {
                    usagePill(label: "1w", window: secondary)
                }
        ...
        private func usagePill(label: String, window: CodexRateLimitWindow) -> some View {
            HStack(spacing: 5) {
                Circle()
                    .fill(CodexUsageStyle.color(for: window).opacity(0.9))
    ...
    private func updateCompletionNoticeState() {
        ...
        if hasActiveNow || hasJSONLBackedActivity {
            ...
            completionDebounceTask = Task {
                ...
                guard shouldNotifyCompletion(for: completed) else { return }
                await MainActor.run {
                    showCompletion(for: completed)
                }
            }
        }
    ...
    private func handleAgentCompletion() {
        ...
        guard let completeCall = visibleToolCalls.first(where: isCompletionToolCall),
              shouldNotifyCompletion(for: completeCall) else { return }

        showCompletion(for: completeCall)
    }

    private func handleCodexTaskCompletion() {
        guard isSourceVisible(.codex),
              let completion = codexManager.lastTaskCompletion else { return }

        let notificationId = "codex:\(completion.id)"
        guard settings.lastNotifiedCompletionId != notificationId else { return }

        let endTime = completion.completedAt
        let startTime: Date
        if let durationMs = completion.durationMs {
        ...
        var toolCall = ToolCall(
            id: UUID(uuidString: completion.id) ?? UUID(),
            toolName: "Complete",
            startTime: startTime,
            source: .codex
        )
        toolCall.endTime = endTime
        toolCall.result = .success(content: completion.lastAgentMessage ?? "")

        showCompletion(for: toolCall, notificationId: notificationId)
    }

    private func shouldNotifyCompletion(for toolCall: ToolCall, now: Date = Date()) -> Bool {
        guard isCompletionToolCall(toolCall),
              let endTime = toolCall.endTime else {
            return false
        }
        guard toolCall.id != lastCompletionId,
              settings.lastNotifiedCompletionId != toolCall.id.uuidString else {
            return false
        }

        return now.timeIntervalSince(endTime) <= completionNotificationFreshness
    }

    private func showCompletion(for toolCall: ToolCall, notificationId: String? = nil) {
        showCompletionNotice = true
        completionToolCall = toolCall
        lastCompletionId = toolCall.id
        settings.lastNotifiedCompletionId = notificationId ?? toolCall.id.uuidString
        playCompletionSound(for: toolCall)
    ...
    private func playCompletionSound(for toolCall: ToolCall) {
        guard settings.playCompletionSound else { return }

        let soundName = toolCall.isSuccess ? "Glass" : "Basso"
