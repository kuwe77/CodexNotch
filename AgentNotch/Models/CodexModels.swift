...
struct CodexTaskCompletion: Identifiable, Equatable {
    let id: String
...
struct CodexRateLimitWindow: Equatable {
    ...
    var formattedUsedPercent: String {
...
struct CodexRateLimits: Equatable {
    ...
    static let empty = CodexRateLimits()
    ...
    static func normalized(
        ...
        case let (.some(primaryWindow), .some(secondaryWindow)):
            ...
            case (.session, .weekly), (.session, .unknown), (.unknown, .weekly):
                return CodexRateLimits(primary: primaryWindow, secondary: secondaryWindow, limitId: limitId)
            case (.weekly, .session), (.weekly, .unknown):
                return CodexRateLimits(primary: secondaryWindow, secondary: primaryWindow, limitId: limitId)
            default:
                return CodexRateLimits(primary: primaryWindow, secondary: secondaryWindow, limitId: limitId)
            }
        ...
        case let (.some(primaryWindow), .none):
            ...
            case .weekly:
                return CodexRateLimits(primary: nil, secondary: primaryWindow, limitId: limitId)
            case .session, .unknown:
                return CodexRateLimits(primary: primaryWindow, secondary: nil, limitId: limitId)
            }

        case let (.none, .some(secondaryWindow)):
            switch role(for: secondaryWindow) {
            case .session, .unknown:
                return CodexRateLimits(primary: secondaryWindow, secondary: nil, limitId: limitId)
            case .weekly:
                return CodexRateLimits(primary: nil, secondary: secondaryWindow, limitId: limitId)
            }
    ...
    private static func role(for window: CodexRateLimitWindow) -> WindowRole {
...
struct CodexState: Equatable {
    ...
    var rateLimits: CodexRateLimits = .empty
